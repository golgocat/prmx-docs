...
> **Status**: Reference (updated 2026-05-01)
> **Source**: Bittensor (Subtensor + Frontier EVM + Hyperlane) および TaoFi の公開実装調査
> **Scope**: PRMX Route 2(Base Sepolia) の Hyperlane-only 運用と、将来拡張時に維持すべき DeFi 運用パターン集
> **Purpose**: Hyperlane Warp Route / Interchain Accounts を本番運用するにあたり、Bittensor エコシステムが実戦で確立した設計・運用パターンのうち、PRMX にそのまま(または適応して)採用すべきものを記録する

> **Current PRMX baseline**: Route 2 is the only active capital route;
> `pallet-assets(1)` is the settlement-balance source of truth;
> `VAULT_REPORTER_TRANSPORT=hyperlane` is live; SBP-420 deployed and verified
> the Morpho Blue testnet strategy. Borrower automation is testnet-only and
> off-book.
...
-> Base ICA proxy -> `IcaCommandRouter` -> `PolicyVaultManager`。SBP-404 cutover
後の直接 Base write は運用違反として扱う。
...
- **Yield 分離**: SBP-436 / SBP-437 により、rebalance credit/debit と funding は yield ではなく capital movement として扱う
...
**現時点の Route 2 testnet** では、MockLendingVault は loss-mode harness として残しつつ、
SBP-420 で Morpho Blue testnet strategy を deployed/verified 済み。これは future-only
ではなく、`VAULT_REPORTER_TRANSPORT=hyperlane` で PRMX 側の vault snapshot へ届ける運用対象。
...
したがって「yield 戦略を外部化する」という思想は参考に留め、PRMX 独自の階層設計を優先する。特に initial funding / rebalance / over-backed surplus は yield ではなく capital movement として quarantine し、zero-baseline report は matching ack が出るまで yield credit しない。Spec 510 以降は `record_vault_funded` が baseline calibration の起点であり、SBP-455 以降は rebalance-in を `uncalibrated_or_unknown` / `already_funded` / `real_drawdown_candidate` に分け、実 drawdown だけを自動 top-up 対象にする。Triggered payout 時の negative strategy P/L は DAO backstop で補填し、`pallet-assets(1)` の未担保 mint で隠さない。
...
Hyperlane ICA は SBP-404 cutover 後の Base operation path として採用済み。ただし、**横断的「アトミック多段呼び出し」は PRMX では採用しない**:

- Rebalancer executor、reserve return、strategy operation は PRMX 側から ICA single-call として発火する
- Base `PolicyVaultManager` / `VaultFactory` への直接 write は cutover 後に禁止
...
(`0x0800` / `0x0801` / `0x0802`)を canonical bridge / exit / yield report に使うが、
Base から Substrate 側を直接呼ぶ運用は採らない(Hyperlane メッセージング経由で十分)。

### なぜ不採用なのか

- PRMX EVM precompile は Route 2 の狭い settlement surface に限定する
...
| フェーズ | 採用項目 | タイミング |
|---------|---------|-----------|
| Current Route 2 testnet | #1, #2, #4, #5, #8, #9(single-call) | Hyperlane-only route / Morpho testnet verified |
...
- `docs/archive/hyperlane-migration-pre-cutover/HYPERLANE-MIGRATION-PLAN-AUDIT-TRAIL.md` — 本ドキュメントの前提となった移行計画 (cutover 後は audit trail)
- `docs/hyperlane-migration/m72-pallet-assets-hyperlane-canonical-path-decision.md` — `pallet-assets(1)` を SSoT とする canonical path 設計 (現行)
- `docs/hyperlane-migration/m75-ica-yield-command-bus.md` — ICA + yield コマンドバス設計 (現行)
